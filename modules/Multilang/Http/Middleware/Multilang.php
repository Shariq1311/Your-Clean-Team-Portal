...
namespace MojarCMS\Multilang\Http\Middleware;
...
class Multilang
{
    ...
    {
        $locale = session()->get('mc_locale');
        if ($locale) {
            return $locale;
        }

        $locale = Cookie::get('mc_locale');
        if ($locale) {
            $this->setLocaleSession($locale);
            return $locale;
        }
    ...
    {
        session()->put('mc_locale', $locale);

        Cookie::queue('mc_locale', $locale, time() + 2592000);
    }
