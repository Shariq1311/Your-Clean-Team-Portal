...
if (!function_exists('set_config')) {
     ...
     * @param string $key
     * @param mixed $value
     * @return \MojarCMS\CMS\Models\Config
     */
    function set_config(string $key, mixed $value): \MojarCMS\CMS\Models\Config
    {
...
if (!function_exists('count_unread_notifications')) {
    ...
    function count_unread_notifications(): int
    {
        ...
        if (method_exists($mc_user, 'unreadNotifications')) {
            return $mc_user->unreadNotifications()->cacheFor(3600)->count(['id']);
        }
...
function user_avatar($user = null): string
{
    ...
    return asset('mc-styles/Mojar/images/thumb-default.png');
}

if (!function_exists('mc_breadcrumb')) {
    function mc_breadcrumb(string $name, array $addItems = []): \Illuminate\Contracts\View\View
    {
...
if (!function_exists('upload_url')) {
    ...
    function upload_url(?string $path, ?string $default = null, ?string $size = null): string
    {
        ...
        if (empty($path) || !$storage->exists(mc_basepath($path))) {
            if ($default) {
                return $default;
            }
        ...
        if ($size) {
            ...
            if ($storage->exists(mc_basepath($pathSize))) {
                return $storage->url($pathSize);
            }

            if (config('Mojar.filemanager.image_resizer')) {
                return asset("mc-styles/images/resize/{$size}/{$path}");
            }
        }
...
if (!function_exists('is_json')) {
    /**
     * Return true if string is a json
     *
     * @param mixed $string
     * @return bool
     */
...
if (!function_exists('mc_date_format')) {
    /**
     * Format date to global format cms
     *
     * @param string $date
     * @param int $format // MC_DATE || MC_DATE_TIME
     * @return string
     */
    function mc_date_format(mixed $date, int $format = MC_DATE_TIME): string
    {
...
if (!function_exists('mc_current_user')) {
    /**
     * Get current login user
     *
     * @return \Illuminate\Contracts\Auth\Authenticatable|User|null
     */
    function mc_current_user(): \Illuminate\Contracts\Auth\Authenticatable|User|null
    {
        return Auth::user();
    }
}

if (!function_exists('mc_get_page')) {
    /**
     * @param $id
     * @return Post|null
     */
    function mc_get_page($id): Post|null
    {
        return Post::find($id);
    }
}
...
if (!function_exists('mc_get_select_options')) {
    function mc_get_select_options($data): array
    {
        $result = [];
...
if (!function_exists('collect_metas')) {
    ...
    {
        return \MojarCMS\CMS\Facades\Field::collect($metas);
    }
}
...
function mc_basename($name): string
{
    $base = basename($name);
    $base = explode('?', $base)[0];
    if (empty($base)) {
...
if (!function_exists('mc_basepath')) {
    function mc_basepath(string $path): string
    {
