...
use MojarCMS\Backend\Http\Resources\ResourceResource;
use MojarCMS\Backend\Models\Taxonomy;
use MojarCMS\Backend\Http\Resources\PostResource;
use MojarCMS\Backend\Models\Post;
use MojarCMS\Backend\Models\Resource;
use MojarCMS\CMS\Facades\MCQuery;
...
{
    return MCQuery::posts($type, $options);
}
...
function get_posts_by_filter(?array $options): ?array
{
    ...
    return MCQuery::posts($options['type'] ?? 'posts', $options);
}

function get_post_taxonomy($post, $taxonomy = null, $params = []): ?array
{
    return MCQuery::postTaxonomy($post, $taxonomy, $params);
}

function get_post_taxonomies($post, $taxonomy = null, $params = []): ?array
{
    return MCQuery::postTaxonomies($post, $taxonomy, $params);
}

function get_related_posts($post, $limit = 5, $taxonomy = null): ?array
{
    return MCQuery::relatedPosts($post, $limit, $taxonomy);
}
