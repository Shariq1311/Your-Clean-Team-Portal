...
class LoadDataController extends BackendController
{
    ...
    protected function loadTaxonomies(Request $request)
    {
        ...
        if ($search) {
            $query->where('name', MC_SQL_LIKE, '%' . $search . '%');
        }
        ...
        if ($search) {
            $query->where(
                function (Builder $q) use ($search) {
                    $q->where('name', MC_SQL_LIKE, '%' . $search . '%');
                    $q->orWhere('email', MC_SQL_LIKE, '%' . $search . '%');
                }
            );
        }
    ...
    protected function loadMenu(Request $request)
    {
        ...
        if ($search) {
            $query->where(
                function (Builder $q) use ($search) {
                    $q->orWhere('name', MC_SQL_LIKE, '%' . $search . '%');
                }
            );
        }
    ...
    protected function loadPages(Request $request)
    {
        ...
        if ($search) {
            $query->where('title', MC_SQL_LIKE, '%' . $search . '%');
        }
