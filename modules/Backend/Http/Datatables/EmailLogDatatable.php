...
namespace MojarCMS\Backend\Http\Datatables;

use Illuminate\Database\Query\Builder;
use Illuminate\Support\Arr;
use MojarCMS\CMS\Abstracts\DataTable;
use MojarCMS\CMS\Jobs\SendEmailJob;
use MojarCMS\Backend\Models\EmailList;
use MojarCMS\CMS\Support\SendEmail;
    ...
    public function columns()
    {
        return [
            ...
            'updated_at' => [
                ...
                'align' => 'center',
                'formatter' => function ($value, $row, $index) {
                    return mc_date_format($row->updated_at);
                },
            ],
        ];
    }
        ...
        if ($search = Arr::get($data, 'keyword')) {
            $query->where(
                function (Builder $q) use ($search) {
                    $q->where('subject', MC_SQL_LIKE, '%' . $search . '%');
                    $q->orWhere('content', MC_SQL_LIKE, '%' . $search . '%');
                }
            );
        }
