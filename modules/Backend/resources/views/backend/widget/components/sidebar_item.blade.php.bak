<div class="card sidebar-item mb-3" id="sidebar-{{ $item->get('key') }}">
    <form action="{{ route('admin.widget.update', [$item->get('key')]) }}" method="post" class="form-ajax">
        ...
        >
            <div class="card-body">
                <div class="dd mc-widget-builder" data-key="{{ $item->get('key') }}">
                    @php $widgets = mc_get_widgets_sidebar($item->get('key')); @endphp
                    <ol class="dd-list">
                        @foreach ($widgets as $key => $widget)
                            @php
                                $widgetData = \MojarCMS\CMS\Facades\HookAction::getWidgets($widget['widget'] ?? 'null');
                            @endphp
                            @if (empty($widgetData))
                                @continue
                            @endif
