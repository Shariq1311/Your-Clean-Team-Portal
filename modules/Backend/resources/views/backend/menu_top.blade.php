    ...
    <h1 class="navbar-brand navbar-brand-autodark">
        <a href="/{{ config('Your Clean Team.admin_prefix') }}">
            @if ($logo = get_config('admin_logo'))
                <img src="{{ upload_url(get_config('admin_logo')) }}" alt="{{ get_config('title', 'Your Clean Team') }}">
            @else
                <img src="{{ asset('mc-styles/Your Clean Team/assets/static/logo.png') }}" width="110" height="32"
    ...
    <div class="navbar-nav flex-row order-md-last">
        ...
        <div class="dropdown me-3 d-none d-md-flex align-items-center">
            @php
                $langs = \Illuminate\Support\Facades\Cache::remember('top_menu_languages', 3600, function () {
                    return app(\MojarCMS\CMS\Support\Manager\TranslationManager::class)
                        ->locale('cms')
                        ->languages();
                });
                $current = $mc_user->language ?? get_config('language', 'en');
            @endphp
        ...
        <div class="nav-item dropdown">
            <a href="#" class="nav-link d-flex lh-1 text-reset p-0" data-bs-toggle="dropdown"
                ...
                <span class="avatar avatar-sm" style="background-image: url({{ $mc_user->getAvatar() }})"></span>
