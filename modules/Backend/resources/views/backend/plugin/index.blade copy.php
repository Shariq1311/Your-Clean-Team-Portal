...
@section('breadcrumb-right')
    <div class="col-auto mb-3">
        <div class="btn-group float-right">
            @if (config('Your Clean Team.plugin.enable_upload'))
...
@section('content')
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        function nameFormatter(value, row, index) {
                                                            ...
                                                            if (row.status == 'active') {
                                                                str +=
                                                                    `<li class="list-inline-item"><a href="javascript:void(0)" class="mc-table-row action-item" data-id="${row.id}" data-action="deactivate">${Your Clean Team.lang.deactivate}</a></li>`;
                                                            } else {
                                                                str +=
                                                                    `<li class="list-inline-item"><a href="javascript:void(0)" class="mc-table-row action-item" data-id="${row.id}" data-action="activate">${Your Clean Team.lang.activate}</a></li>`;
                                                            }

                                                            if (row.setting && row.status == 'active') {
                                                                str +=
                                                                    `<li class="list-inline-item"><a href="${Your Clean Team.adminUrl +'/'+row.setting}" class="mc-table-row">${Your Clean Team.lang.setting}</a></li>`;
                                                            }

                                                            @if (config('Your Clean Team.plugin.enable_upload'))
                                                                if (row.update) {
                                                                    str +=
                                                                        `<li class="list-inline-item"><a href="javascript:void(0)" class="mc-table-row action-item" data-id="${row.id}" data-action="update">${Your Clean Team.lang.update}</a></li>`;
                                                                }

                                                                str +=
                                                                    `<li class="list-inline-item"><a href="javascript:void(0)" class="mc-table-row text-danger action-item" data-id="${row.id}" data-action="delete">${Your Clean Team.lang.delete}</a></li>`;
                                                            @endif
                                                            str += `</ul>`;
                                                            return str;
                                                        }
                                                        ...
                                                        function statusFormatter(value, row, index) {
                                                            switch (value) {
                                                                case 'inactive':
                                                                    return `<span class='text-disable'>${Your Clean Team.lang.inactive}</span>`;
                                                            }

                                                            return `<span class='text-success'>${Your Clean Team.lang.active}</span>`;
                                                        }

                                                        var table = new Your Clean TeamTable({
    ...
    <script>
        $(function() {
            ...
            function showNotification(response) {
                if (typeof Your Clean Teamcms !== 'undefined' && typeof Your Clean Teamcms.message !== 'undefined') {
                    Your Clean Teamcms.message(response);
                } else {
                    const toastHtml = `
    ...
    {{-- <div class="table-responsive mb-5">
        <table class="table mc-table Your Clean Team-table">
            <thead>
                <tr>
