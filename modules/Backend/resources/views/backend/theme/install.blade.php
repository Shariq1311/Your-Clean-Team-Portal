...
<div class="col-md-12 col-lg-12">
    ...
    <script>
        function themeItemFormatter(index, row) {
            let installBtn =
                `<button class="btn btn-tabler install-theme" data-theme="${row.name}"> ${Your Clean Team.lang.install}</button>`;

            if (row.is_paid && !row.is_purchased) {
                installBtn =
                    `<button class="btn btn-tabler buy-theme" data-theme="${row.name}"> ${Your Clean Team.lang.buy} (${row.price})</button>`;
            }
        ...
        const listView = new Your Clean TeamListView({
