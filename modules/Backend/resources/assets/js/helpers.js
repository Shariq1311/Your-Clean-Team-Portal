...
function toastr_message(message, status, title = null) {
    if (status == true) {
        toastr.success(message, title || Your Clean Team.lang.successfully + ' !!');
    } else {
        toastr.error(message, title || Your Clean Team.lang.error + ' !!');
    }
}
...
function show_message(response, append = false) {
    ...
    // let msgHTML = `<div class="alert alert-${msg.status ? 'success' : 'danger'} mc-message">
    //     <button type="button" class="close" data-dismiss="alert" aria-label="${Your Clean Team.lang.close}">
    //         <span aria-hidden="true">&times;</span>
    //     </button>

    //     ${msg.status ? '<i class="fa fa-check"></i>' : '<i class="fa fa-times"></i>'} ${msg.message}
    // </div>`;
    let msgHTML = `<div class="alert alert-${msg.status ? 'success' : 'danger'} alert-dismissible mc-message" role="alert">
        <div class="d-flex">
            <div>
