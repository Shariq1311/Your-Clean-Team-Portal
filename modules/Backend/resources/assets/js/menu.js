$(document).ready(function () {
    ...
    $('#mc-menu-builder').nestable({
        noDragClass: 'dd-nodrag',
    }).on('change', updateOutput);

    updateOutput($('#mc-menu-builder'));
    ...
    $('#menu-container').on('submit', '.form-menu-block', function (event) {
        ...
        $.ajax({
            ...
            if (items.length > 0) {
                $('#mc-menu-builder .dd-empty').remove();

                $.each(items, function (index, item) {
                    $('#mc-menu-builder .dd-list:first').append(item);
                });
            }
    ...
    $('#menu-container').on('change', '.form-select-menu .load-menu', function () {
        let id = $(this).val();
        if (id) {
            window.location = Your Clean Team.adminUrl + "/menus/" + id;
        }
    });
    ...
    $('#menu-container').on('click', '.delete-menu', function (e) {
        ...
        confirm_message(
            Your Clean Team.lang.remove_question.replace(':name', Your Clean Team.lang.menu),
            function (result) {
                if (result) {
                    ajaxRequest(
                        Your Clean Team.adminUrl + "/menus/" + id,
                        {},
                        {
                            'method': 'DELETE',
                            'callback': function (response) {
                                show_message(response);
                                window.location = Your Clean Team.adminUrl + "/menus";
                            }
                        }
                    );
                }
    ...
    $('#menu-container').on('keyup', '.menu-box-post-type-search', function () {
        ...
        ajaxRequest(Your Clean Team.adminUrl + '/load-data/loadPostType', {
            search: search,
            per_page: 5,
            post_type: postType
        }, {
    ...
    $('#menu-container').on('keyup', '.menu-box-taxonomy-search', function () {
        ...
        ajaxRequest(Your Clean Team.adminUrl + '/load-data/loadTaxonomies', {
            search: search,
            per_page: 5,
            taxonomy: taxonomy
        }, {
