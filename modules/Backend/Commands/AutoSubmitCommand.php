...
{
    protected $signature = 'Mojarcms:auto-submit';
    ...
    public function handle(): int
    {
        ...
        //set_config('mc_last_seo_ping', date('Y-m-d H:i'));

        if (get_config('mc_auto_ping_google_sitemap')) {
            try {
        ...
        if (get_config('mc_auto_submit_url_google')) {
            try {
                for ($i = 1; $i <= 2; $i++) {
        ...
        if (get_config('mc_auto_submit_url_bing')) {
            try {
                $this->submitUrlBing();
            } catch (\Exception $e) {
                report($e);
    ...
    {
        $lastDate = get_config('mc_last_google_submit_date', '2000-01-01 00:00:00');
        ...
        set_config('mc_last_google_submit_date', $lastUpdatedAt);
    ...
    protected function submitUrlBing(): void
    {
        ...
        $lastDate = get_config('mc_last_bing_submit_date', '2000-01-01 00:00:00');
        ...
        if ($response->getStatusCode() == 200) {
            ...
            set_config('mc_last_bing_submit_date', $links->last()->updated_at->format('Y-m-d H:i:s'));
        }
    }

    protected function checkSubmitDate(): bool
    {
        $lastPing = get_config('mc_last_seo_ping');
    ...
    {
        return get_config('mc_bing_api_key');
    }
