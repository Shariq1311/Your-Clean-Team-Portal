...
namespace MojarCMS\CMS\Traits\Auth;

use Illuminate\Contracts\View\View;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\RedirectResponse;
use Illuminate\Support\Facades\Auth;
use MojarCMS\CMS\Http\Requests\Auth\LoginRequest;
use MojarCMS\CMS\Models\User;
use MojarCMS\CMS\Traits\ResponseMessage;
...
trait AuthLoginForm
{
    ...
    public function login(LoginRequest $request): JsonResponse|RedirectResponse
    {
        ...
        if ($user->status == 'unconfirmed') {
            return $this->error(
                [
                    'message' => trans('cms::message.login_form.user_is_unconfirmed'),
                ]
            );
        }
        ...
        if ($userType === 'vendor') {
            ...
            if ($vendorStatus === 'under_review') {
                return $this->error(
                    [
                        'message' => trans('ecomm::content.login_form.vendor_under_review'),
                    ]
                );
            }
            ...
            if ($vendorStatus === 'rejected') {
                return $this->error(
                    [
                        'message' => trans('ecomm::content.login_form.vendor_rejected'),
                    ]
                );
            }
            ...
            if ($vendorStatus === 'suspended') {
                return $this->error(
                    [
                        'message' => trans('ecomm::content.login_form.vendor_suspended'),
                    ]
                );
            }
        }
