...
use MojarCMS\CMS\Facades\GlobalData;
...
trait GetHookAction
{
    ...
    {
        $result = GlobalData::get('menu_boxs.' . $key);

        // Check if the result is an array and convert it to a Collection
        if (is_array($result)) {
            return collect($result); // Return as Collection
        }

        // If it's not an array, return it as is (bool or other types can be returned)
        return $result;
    }
