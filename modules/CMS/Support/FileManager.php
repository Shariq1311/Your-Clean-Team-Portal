...
use MojarCMS\Backend\Events\MediaWasUploaded;
use MojarCMS\CMS\Exceptions\FileManagerException;
...
class FileManager
{
    ...
    public function save(): MediaFile
    {
        ...
        if ($this->resource_type != 'url' || $this->downloadFileUrlToServer) {
            ...
            $fileName = mc_basename($this->resource);
            ...
            $media = MediaFile::create(
                [
                    ...
                    'user_id' => $this->user_id ?: $mc_user->id,
                    'disk' => $this->disk,
                ]
            );
        }
    ...
    {
        return preg_replace('/\.(php\d*|phtml|pht|phar)(\..*)?$/i', '', $name);
    }
