...
<div class="card mb-4">
    <div class="card-header">
        <h5 class="card-title">{{ __('Commission Settings') }}</h5>
        <div class="card-subtitle">
            {{ __('Configure commission rates and withdrawal fees for vendors.') }}
        </div>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-6">
                {{ Field::text('Default Commission Rate (%)', 'ecomm_default_commission_rate', [
                    'type' => 'number',
                    'value' => get_config('ecomm_default_commission_rate', 10),
                    'class' => 'form-control',
                    'min' => 0,
                    'max' => 100,
                    'step' => 0.01,
                    'help' => __('Default commission rate applied to vendor sales.'),
                ]) }}
            </div>
            <div class="col-md-6">
                {{ Field::select('Withdrawal Fee Type', 'ecomm_withdrawal_fee_type', [
                    'value' => get_config('ecomm_withdrawal_fee_type', 'percentage'),
                    'options' => [
                        'percentage' => 'Percentage (%)',
                        'fixed' => 'Fixed Amount',
                    ],
                    'class' => 'form-select',
                    'help' => __('Type of fee charged for withdrawals.'),
                ]) }}
            </div>
        </div>
        ...
        <div class="row mt-3">
            <div class="col-md-6">
                {{ Field::text('Withdrawal Fee Amount', 'ecomm_withdrawal_fee_amount', [
                    'type' => 'number',
                    'value' => get_config('ecomm_withdrawal_fee_amount', 2.5),
                    'class' => 'form-control',
                    'min' => 0,
                    'step' => 0.01,
                    'help' => __('Fee amount for withdrawals (percentage or fixed amount).'),
                ]) }}
            </div>
        </div>
    </div>
