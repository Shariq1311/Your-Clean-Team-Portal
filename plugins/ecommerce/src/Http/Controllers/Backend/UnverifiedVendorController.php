...
use Mojahid\Ecommerce\Http\Datatables\UnverifiedVendorDatatable;

class UnverifiedVendorController extends BackendController
{
    ...
    protected string $viewPrefix = 'ecomm::backend.unverified-vendor';
    ...
    {
        return trans('ecomm::content.unverified_vendors');
    }
     ...
     *
     * @param mixed ...$params
     * @return UnverifiedVendorDatatable|DataTable
     */
    protected function getDataTable(...$params): UnverifiedVendorDatatable|DataTable
    {
        return new UnverifiedVendorDatatable();
    }

    protected function getDetailModel($model, ...$params)
    {
        $user = $model
            ->where('id', $this->getPathId($params))
            ->firstOrFail();
            
        // Ensure the model has the correct status
        return $user;
    }
    ...
    protected function getDataForForm($model, ...$params): array
    {
        ...
        $data['vendor_status'] = $model->getMeta('user_status', 'under_review');
    ...
    protected function afterSave($data, $model, ...$params)
    {
        ...
        if ($vendorStatus) {
            $originalStatus = $model->getMeta('user_status', 'under_review');
            ...
            if ($originalStatus !== $vendorStatus) {
                Log::info('Triggering user.after_save hook from UnverifiedVendorController', [
                    'user_id' => $model->id,
