...
class UnverifiedVendorDatatable extends DataTable
{
    /**
     * Columns datatable
     *
    ...
    public function columns(): array
    {
        return [
            ...
            'business_address' => [
                'label' => trans('ecomm::content.business_address'),
                'width' => '15%',
                'align' => 'center',
                'formatter' => function ($value, $row, $index) {
                    return $row->getMeta('business_address', '-');
                },
            ],
            'vendor_status' => [
                ...
                'align' => 'center',
                'formatter' => function ($value, $row, $index) {
                    $status = $row->getMeta('user_status', 'under_review');
                    $statusLabels = [
                    ...
                    return $statusLabels[$status] ?? '<span class="badge bg-warning">' . trans('ecomm::content.under_review') . '</span>';
                },
            ],
            'email' => [
    ...
    public function rowAction($row)
    {
        ...
        $data['edit'] = [
            ...
            'url' => route('admin.unverified-vendors.edit', [$row->id]),
        ];
    ...
    public function query($data)
    {
        ...
        // Filter by vendor status - show under_review and rejected vendors
        $query->whereHas('metas', function (Builder $q) {
            $q->where('meta_key', 'user_status')
              ->whereIn('meta_value', ['under_review', 'rejected']);
        });
