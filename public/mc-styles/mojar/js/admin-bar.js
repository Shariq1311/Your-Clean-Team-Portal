document.addEventListener('DOMContentLoaded', () => {
    const speedDial     = document.getElementById('mcSpeedDial');
    const toggleBtn     = document.getElementById('mcSpeedDialToggle');
    const actionsList   = document.getElementById('mcSpeedDialActions');
    let isOpen          = false;
    ...
    // --- Enhanced Drag & Drop ---
    const dialItems = document.querySelectorAll('.mc-speed-dial-item');
    ...
    function setupItem(item) {
        const closeBtn = item.querySelector('.mc-close-item');
    ...
    function getDragAfterElement(container, y) {
        const draggableElements = [
            ...container.querySelectorAll('.mc-speed-dial-item:not(.hide):not(.dragging)')
        ];
    ...
    function saveSpeedDialOrder() {
        const items = document.querySelectorAll('.mc-speed-dial-item');
        const order = [];
        items.forEach((el) => {
            order.push(el.dataset.itemId);
        });
        localStorage.setItem('mcSpeedDialOrder', JSON.stringify(order));
    }

    function loadSpeedDialOrder() {
        const storedOrder = JSON.parse(localStorage.getItem('mcSpeedDialOrder') || '[]');
        if (storedOrder.length) {
            const itemsMap = {};
            document.querySelectorAll('.mc-speed-dial-item').forEach((el) => {
    ...
    actionsList.addEventListener('keydown', (e) => {
        ...
        const focusableItems = [...actionsList.querySelectorAll('.mc-speed-dial-item')];
    ...
    searchInput.className = 'mc-speed-dial-search';
    ...
    const speedDialLinks = document.querySelectorAll('.mc-speed-dial-link');
    speedDialLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            // Add loading state
            const item = link.closest('.mc-speed-dial-item');
